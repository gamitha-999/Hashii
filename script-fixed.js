...
(function(){
  ...
  // UI bindings
  function bindUI(){ $('#join-btn').addEventListener('click', ()=>{ const n=$('#name-input').value.trim(); if(!n){ toast('Enter your name'); return; } joinGame(n); }); $('#leave-btn').addEventListener('click', ()=>{ if(playerId) db.ref(`games/${GAME_ID}/players/${playerId}`).update({ connected:false, lastSeen: now() }); show('#join-screen'); }); $('#leave-game-btn').addEventListener('click', ()=>{ if(playerId) db.ref(`games/${GAME_ID}/players/${playerId}`).update({ connected:false, lastSeen: now() }); show('#join-screen'); }); $('#back-lobby-btn').addEventListener('click', ()=>{ metaRef().set({ status: 'lobby' }); show('#lobby-screen'); }); $('#reset-local-btn').addEventListener('click', ()=>{ localStorage.clear(); toast('Local data reset'); location.reload(); });
    // Admin login button
